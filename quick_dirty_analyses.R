...
case_counts_raw = cases_with_pop[,30:183]
...
case_counts <- case_counts[case_counts[,153] !=0,]
...
for (i in 1:nrow(case_counts)){ # we are going to iterate over all the states
  for (j in 1:21){ #add cases to be cumulative across the week
    ...
    if (sum(x)<=10) { #cutoff is now less than 10 cases in a week
      ...
      R0EG = estimate.R(case_counts[i,], GT=mGT,  methods=c('EG'), pop.size = case_counts[,145], nsim=10, begin=weekly[j]+1, end=weekly[j+1])
...
Week <- as.character(c(1:21))
dates <- colnames(case_counts[,c(1,8,15,22,29,36,43,50,57,64,71,78,85,92,99,106,113,120,127,134, 141)])
...
corr_apple2[,7][is.na(corr_apple2[,7])] <- FALSE
...
total_data$expected_decrease <- base::rowMeans(total_data[,c(17,16,14,13,12,11)], na.rm=T)
...
total_data$pct_change[total_data$pct_change>quantile(total_data$pct_change, .95, na.rm=T)] <- NA
total_data$pct_change[total_data$pct_change<quantile(total_data$pct_change, .05, na.rm=T)] <- NA
