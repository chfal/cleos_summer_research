...
total_data %>%
  group_by(state) %>%
  filter(state %in% c("California","Connecticut","Delaware","Hawaii","Illinois","Maine","Maryland","Massachusetts","Michigan","New Jersey","New Mexico","New York","North Carolina","Pennsylvania", "Puerto Rico","Rhode Island","Virginia","Washington")) %>%
  ggplot(aes(x=masks_worn, y=lagged_R0, fill=masks_worn)) +
  geom_boxplot() +
  theme(legend.position="none") +
  facet_wrap(~state) +
  scale_x_discrete(labels=c("No Masks","Masks")) +
  labs(x="Masks Worn?", y="R0 with 2-week lag",title="R0 (2-week lag) Before and After Mask Wearing") +
  ggsave("masks_r0.jpeg")
```
